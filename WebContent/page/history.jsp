...
<body>
	<form method="post" action="http://120.55.40.199:80/yyt_app/MainServlet">
		<input type="text" name="method" value="searchOrderWeb"
			style="display: none">
		<div class="demoTable">
			关键字查询：
...
/****************************************************************/
				...
				$(".layui-btn").click(function(){
					...
					table.render({
						...
						where: {method: 'searchOrderWeb',keyword:name}, //如果无需传递额外参数，可不加该参数
					...
					table.render({
						...
						elem: "#demo",
						url :'http://120.55.40.199:80/yyt_app/WebMainServlet',
						where: {method: 'searchAllOrdersWeb'}, //如果无需传递额外参数，可不加该参数
						method: 'post' ,//如果无需自定义HTTP类型，可不加该参数
						cols: [
							[
					...
					if(layEvent === 'detail') { //查看
						var datastring = JSON.stringify(data);
							...
							parent.layer.open({
							title: '<div style="font-size:x-large"><b>交易详情</b></div>',
							type: 2,
							area: ['580px', '600px'],
							fixed: false, //不固定
							maxmin: true,							
							content: urlRootContext + '/view/nav/Order.html'
							});
						layer.full(index);
					}
					...
					if(layEvent === 'del') { //删除
					 //删除商户
					 	parent.layer.confirm('删除之后，数据库中将没有该条记录', function(index){
	                		...
	                		url: "http://120.55.40.199:80/yyt_app/WebMainServlet",
	                		type: "POST",
	                		data:{method: 'deleteOrderByIdWeb',"order_id":data.order_id},
	                		dataType: "json",
